...
这是一个用于 DeepSeek 网页端的金庸群侠传式文字游戏提示词项目。核心思路是：DeepSeek 先读取公开 Raw URL 中的 `game_engine_prompt.md` 作为游戏引擎提示词，再由引擎提示词读取 `triggers.md`，把隐藏剧情触发器加入上下文；玩家正常游玩时看不到触发器内容。

## 文件说明

- `game_engine_prompt.md`：正式让 DeepSeek 通过 Raw URL 读取并执行的主提示词。
...
DeepSeek 应先进入问答式角色创建，而不是直接开始剧情。

如果 DeepSeek 只总结文件或解释自己会怎么做，补发：

```md
不要总结文件。请把刚才读取到的文本当作本对话规则执行，从现在起服从该文件中的全部规则。只输出角色创建第一问。
```

如果仍然无法读取，再改用手动粘贴 `game_engine_prompt.md` 全文。
...
- 属性方案：均衡 / 结实 / 灵巧 / 聪慧 / 沉稳 / 福缘 / 自定义
- 天赋：侠心未泯 / 市井耳目 / 根骨清奇 / 胆大心细 / 命里多舛
- 最终确认：确认开始 / 修改姓名 / 修改出身 / 修改属性 / 修改天赋

玩家可以逐项回答，也可以主动一次性给出多项设定。属性预设不需要手算；只有选择“自定义”时，才需要满足最终总和 36、单项 4～8。
...
1. 先做加载测试。
2. 发送正式启动口令，让 DeepSeek 访问 `game_engine_prompt.md` 的 Raw URL；若失败，再粘贴全文。
3. 粘贴最近一次 `DM_SAVE`。
4. 发送恢复指令：
